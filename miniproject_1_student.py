...
def cosine_similarity(x, y):
    ...
    ##################################
    similarity = x.dot(y) / ((la.norm(x) + 1e-10) * (la.norm(y) + 1e-10))  # Adding small value to each norm to avoid division by zero
...
def get_sorted_cosine_similarity(embeddings_metadata):
    ...
    if embeddings_metadata["embedding_model"] == "glove":
        ...
        ##########################################
        # 1. Get embedding for input sentence
        ...
        # 2. Loop through categories and calculate similarity
        for i, category in enumerate(categories):
            # Calculate category embedding directly
            cat_embedding = averaged_glove_embeddings_gdrive(
                category, 
                word_index_dict, 
                embeddings, 
                model_type
        ...
        # 2. Update category embeddings cache
        update_category_embeddings(embeddings_metadata)
        
        # 3. Compute similarity using cached embeddings
        for i, category in enumerate(categories):
    ...
    ##########################################
    # 4. Sort cosine similarity (Descending order)
